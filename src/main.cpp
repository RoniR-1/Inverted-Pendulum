...
#define RAW_ANGLE_MSB 0x0C  // i2c-> A4 sda. A5 scl
...
// ============================================================================
//      TORQUE-OPTIMIZED 24V PARAMETERS (NEMA17 42BYGH47-401A)
// ============================================================================
// Pendulum Target & Safety
float targetAngle   = 192.67;     // Vertical center offset
const float safeMargin = 25.0;    // Shut down motor if tilt > 25 deg
float deadbandAngle = 0.5f;       // Prevents micro-hunting at center

// --- DYNAMIC TORQUE BOUNDARIES ---
float speedMultiplier = 900.0f;      // Proportional speed scaling (prevents instant speed pegging)
float maxAccelPerLoop = 80.0f;       // 80 steps/loop = 32,000 steps/s² (smooth acceleration)
float minSpeedStepsSec = 100.0f;     // Minimum starting step rate
float maxSpeedStepsSec = 14000.0f;   // ~525 RPM max speed (peak torque band for belt-driven cart)
float lowAngleBoostExponent = 1.0f;  // Pure linear scaling

// --- STABLE CONTROL GAINS ---
float Kp = 40.0f;   // Responsive baseline gain
float Ki = 0.00f;   // Keep disabled
float Kd = 2.10f;   // High-speed derivative damping
// ============================================================================
...
void setup() {
  ...
  Serial.println(status, HEX); // 0x20 = Magnet OK, 0x28 = Magnet too close, 0x30 = Magnet too far
  if (status & 0x20) Serial.println("AS5600: Magnet detected!");
    else Serial.println("AS5600: WARNING - NO MAGNET DETECTED!");
